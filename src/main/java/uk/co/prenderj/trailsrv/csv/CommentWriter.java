...
public class CommentWriter extends CSVWriter {
	...
	public void writeNextComment(Comment comment) {
		writeNext(new String[] {String.valueOf(comment.id), String.valueOf(comment.longitude), String.valueOf(comment.latitude),
				comment.title, comment.body, comment.timestamp.toString()});
	}
}
